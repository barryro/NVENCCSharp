...
//20170622
NVENCSTATUS CNvHWEncoder::CreateEncoder2(EncodeConfig *pEncCfg)
{
	NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
...
NVENCSTATUS CNvHWEncoder::ProcessOutput2(const EncodeBuffer *pEncodeBuffer, NV_ENC_LOCK_BITSTREAM &lockBitstreamData)
{
	NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
...
bool  CNvHWEncoder::GetLibraryFromManaged(HINSTANCE intPtr, MYPROC proc)
{
	...
	m_hinstLib = intPtr;
	printf("\n m_hinstLib = %p", m_hinstLib);
	//memcpy(m_nvEncodeAPICreateInstance, proc, sizeof(MYPROC));
